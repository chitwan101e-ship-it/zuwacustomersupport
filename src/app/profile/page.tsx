...
export default function ProfilePage() {
  ...
  const [appearance, setAppearance] = useState<'dark' | 'light' | 'playful'>('dark')
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('id, role, username, first_name, last_name, created_at')
  ...
  const appearanceOptions = [
    ...
    { id: 'playful' as const, label: 'Playful', icon: Sparkles },
  ]
    ...
    <div className="min-h-screen bg-[#050814] text-white">
      ...
      <main className="max-w-md mx-auto px-4 pb-10">
        <div className="mt-4 flex items-center gap-4">
          ...
          <div>
            <h2 className="text-4xl font-bold leading-none mb-1">{displayName}</h2>
            <p className="text-[#7f8bad] text-sm">{email || `${profile.username}@relay.app`}</p>
        ...
        <section className="mt-7 bg-[#0b1020]/95 border border-white/10 rounded-3xl p-5">
          <p className="text-[#7f8bad] text-xs font-semibold uppercase tracking-wider mb-4">Appearance</p>
        ...
        <section className="mt-5 bg-[#0b1020]/95 border border-white/10 rounded-3xl p-5">
          <p className="text-[#7f8bad] text-xs font-semibold uppercase tracking-wider mb-4">Account</p>
          <div className="space-y-3 text-sm">
            <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
              <span className="text-[#7f8bad]">Member since</span>
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
              <span className="text-[#7f8bad]">Status</span>
              <span className="font-semibold text-white">Approved</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[#7f8bad]">Role</span>
        ...
        <button
          ...
          className="w-full mt-8 py-3 rounded-xl text-[#7f8bad] hover:text-white hover:bg-white/5 transition-colors font-semibold"
        >
          Sign Out
        </button>
      </main>
